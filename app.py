...
def parse_search_keywords(query: str) -> tuple[list[str], list[str]]:
    ...
    Comma-separated tokens with AND logic.
    Minus prefix for excludes: "-philo" removes matches.
    Example: "comp, phys, -philo" -> include=["comp","phys"], exclude=["philo"]
    """
...
def main():
    ...
    with st.sidebar:
        ...
        course_search = st.text_input(
            ...
            placeholder="e.g. comp, phys, -philo"
        )
        st.caption("Comma-separated AND logic. Prefix with - to exclude.")
